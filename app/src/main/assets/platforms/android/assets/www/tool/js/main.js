    ...
    function createRepoAndWriteFile(myUser, myRepo, repoName){
        ...
        myUser.createRepo({"name": repoName}, function(err, res) {
            ...
            var request = new XMLHttpRequest();
                request.open("GET", files[3].path, true);
            ...
            for(var i in files){
                ...
                var request = new XMLHttpRequest();
                request.open("GET", files[i].path, true);
                request.responseType = 'arraybuffer';
                request.myIdx = i;
                request.onload = function(){
                    var uInt8Array = new Uint8Array(this.response);
                    ...
                    var base64 = window.btoa(data);
                    myRepo.write(branchName,
                                 files[this.myIdx].path.substring(Android.getProjectPath().length), // file path + name.
                                 base64, // file content.
                                 'Generated by MOBSITE, '+new Date().toDateString(), // commit message.
                                 function(err) {
                                     // completed deploy task!
