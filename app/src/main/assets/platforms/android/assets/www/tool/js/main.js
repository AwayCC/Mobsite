    ...
    function createRepoAndWriteFile(myUser, myRepo, repoName){
        ...
        myUser.createRepo({"name": repoName}, function(err, res) {
            ...
            for(var idx in files){
                status.push({working: false, success: false});
            }
            console.log("test "+status[0].working+" "+status[0].success);
            var runner = 0;
            var failedFiles = [];
            
            for(var idx=0;idx < files.length;idx++){
                if(!status[idx].success && !status[idx].working ){
                    console.log(idx+" : "+files[idx].path);
                    var request = new XMLHttpRequest();
                    request.open("GET", files[idx].path, true);
                    request.responseType = 'arraybuffer';
                    request.timeout = 10000;
                    request.myIdx = idx;
                    request.myPath = JSON.parse(JSON.stringify(files[idx].path.substring(omitLen)));
                    ...
                    request.onload = function(){
                        ...
                        myRepo.write(branchName,
                                     ...
                                     function(err) {
                                         ...
                                         if(err){
                                             console.log("there is error at "+targetIdx+" : "+err.message);
                                             failedFiles.push{path: targetPath};
                                         }else{
            ...
            */
            function deployFiles(files, omitLen){
                console.log("deploy with "+files.length+" files");
                var successCount = 0;
                var runner = 0;
                var status = [];
                for(var idx in files){
                               ...
                               binaryString[i] = String.fromCharCode(uInt8Array[i]);
                                ...
                                myRepo.write(branchName,
                                             ...
                                             function(err) {
                                                 ...
                                                 status[targetIdx].working = false;
                                                 if(err){
                                                     console.log("there is error at "+targetIdx+" : ");
                                                     failedFiles.push({path: targetPath});
                                                 }else{
                                                     status[targetIdx].success = true;
                                                     successCount++;
                                                 ...
                                                 if(runner == files.length){
                                                     deployFiles(failedFiles, omitLen);
                                                 }
