    ...
    function createRepoAndWriteFile(myUser, myRepo, repoName){
        ...
        myUser.createRepo({"name": repoName}, function(err, res) {
            ...
            var request = new XMLHttpRequest();
                request.open("GET", files[files.length-1].path, true);
                request.responseType = 'arraybuffer';
                request.myIdx = files.length-1;
                request.onload = function(){
                    ...
                    myRepo.write(branchName,
                                 files[this.myIdx].path, // file path + name.
                                 base64, // file content.
                                 'Generated by MOBSITE, '+new Date().toDateString(), // commit message.
                                 function(err) {
                                     // completed deploy task!
                                     //dismissDialogs();
                                     console.log("finish "+files[request.myIdx].path);
                                     if(this.myIdx == files.length-1)
                                         GHloadCircle.style.visibility = "hidden";
                                 }
                    );
                }
            ...
            */
            ...
            console.log(Android.getProjectPath+"/img/img1.jpg");
            var request = new XMLHttpRequest();
